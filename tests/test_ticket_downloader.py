...
def test_mock_rt_responses_structure(mock_rt_responses):
    ...
    for endpoint in expected_endpoints:
        ...
        assert mock_rt_responses[endpoint].startswith(b"RT/"), f"Invalid RT response format for: {endpoint}"
...
def test_mock_session_get_requests(mock_session):
    ...
    response = mock_session.get("https://rt.example.com/REST/1.0/ticket/123/attachments")
    assert response.status_code == 200
    assert b"456: (Unnamed)" in response.content
    
    # Test attachment content
    response = mock_session.get("https://rt.example.com/REST/1.0/ticket/123/attachments/789/content")
    assert response.status_code == 200
...
def test_download_ticket_automation(mock_session):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Should have 2 non-email attachments (456, 789, 790 are excluded as outgoing emails)
        # Only 800 and 801 should be downloaded as they don't have X-RT-Loop-Prevention headers
